 ...
 * DOING:
   ...
   * In ```/workspace/ssd``` of the container, test training: 
     ...
     * Try again with ```bash ./examples/SSD300_FP16_1GPU.sh . ./COCO --save ./SSD_CHECKPOINT``` (for 1 GPU)
       ...
       * I stopped it.
     * At Jul 13 14:48 (Bach time), re-start with ```bash ./examples/SSD300_FP16_8GPU.sh . ./COCO --epochs 20 --save /sandbox > /sandbox/log``` so that I'll have the output saved in host ```~/2022/sandbox```. The estimated time is logged (in nvidia code).
